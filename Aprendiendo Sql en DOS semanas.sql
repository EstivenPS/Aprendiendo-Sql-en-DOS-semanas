...
insert into agenda(apellido, nombre, domicilio, telefono) values('Torres', 'Juan', 'Avellaneda 123', '4458787');
...
insert into empleados(nombre, documento, sexo, domicilio, sueldobasico) values('Bartolome', '27888999', 'm', 'Urquiza 479', 800);
...
insert into empleados(nombre, documento, sexo, domicilio, sueldobasico) values('Bartolome', '31222333', 'm', 'Urquiza 479', 850);
...
insert into agenda(apellido, nombre, domicilio, telefono) values('Salas', 'Susana', 'Paz 1234', '4123456');
...
insert into peliculas(codigo, titulo, actor, duracion) values(5, null, 'R. Gere-J. Roberts', 190);
...
insert into libros(codigo, titulo, autor, editorial) values(3, 'Aprende PHP', 'Mario Molina', 'Nuevo Siglo');
insert into libros(codigo, titulo, autor, editorial) values(1, 'Padre rico, padre pobre', 'Katherin Gomez', 'El universo');
...
insert into alumnos(legajo, documento, nombre, domicilio) values('A267', '22345345', 'Gomez Carlos', 'Urquiza 333');
insert into alumnos(legajo, documento, nombre, domicilio) values('A233', null, 'Perez Mariana', 'Colon 234');
...
insert into medicamentos(nombre, laboratorio, precio, cantidad) values('Amoxidal', 'Bayer', 15.60, 100);

select * from medicamentos;

insert into medicamentos(codigo, nombre, laboratorio, precio, cantidad) values(11, 'alka seltzer', 'Bayer', 25, 50);

update medicamentos set codigo = 4 where nombre = 'Sertal';
...
update peliculas set codigo = 11 where titulo = 'La vida es bella';

delete from peliculas where titulo = 'La vida es bella';

insert into peliculas(titulo, actor, duracion) values('Back to the future', 'Michael J. Fox', 116);
...
insert into medicamentos(nombre, laboratorio, precio, cantidad) values('Amoxilina 500','Bayer',15.60,100);
insert into medicamentos(nombre, laboratorio, precio, cantidad) values(12, 'Amoxidal 500', 'Bayer', 15.60, 100);
...
insert into peliculas(codigo, titulo, actor, duracion) values(11, 'Mision imposible 3', 'Tom Cruise', 120);
insert into peliculas(codigo, titulo, actor, duracion) values(56, 'Mision imposible 3', 'Tom Cruise', 120);
...
insert into clientes(documento, apellido, nombre, domicilio, telefono) values('2333344', 'Perez', 'Ana', 'Colon 234', '1247398');
...
insert into empleados(nombre, documento, sexo, domicilio, sueldobasico, cantidadhijos) values('Ana Acosta', '24555666', 'f', 'Colon 134', 8500000000000, 0);
...
insert into cuentas values('35738573', 1000.25);
insert into cuentas(numero, documento, nombre, saldo) values(5, '28999777', 'Luis Lopez', 34000);
insert into cuentas(numero, documento, nombre) values(5, '28999777', 'Luis Lopez', 34000);
insert into cuentas(numero, documento, nombre, saldo) values ('25666777', 'Juan Perez', 2500.50);
...
insert into visitantes values('Estiven Padilla', 18, 'm', 'Cenovi', default, '8092908208', default, 0);
...
select count(precio) as 'Precio distinto de null', count(cantidad) as 'Cantidad distinto de null' from medicamentos;
...
select datepart(year,fechaingreso), count(*) from empleados group by datepart(year,fechaingreso);
select seccion, avg(sueldo) as 'promedio de sueldo' from empleados where cantidadhijos > 0 and cantidadhijos is not null group by seccion;
...
select ciudad, sexo, sum(montocompra) as 'Total' from visitantes where montocompra > 50 and telefono is not null group by all ciudad, sexo having ciudad <> 'Cordoba' order by ciudad;
select ciudad, max(montocompra) from visitantes where sexo = 'f' and domicilio is not null group by all ciudad having max(montocompra) > 50
select ciudad, sexo, count(*) as cantidad, sum(montocompra) as total, avg(montocompra) as promedio from visitantes group by ciudad, sexo having avg(montocompra) > 30 order by total;
...
--41 - Combinación interna (inner join)
--Primer problema:
if object_id('clientes') is not null
	drop table clientes;
...
select nombre, insc.deporte, ina.fecha from inscriptos as insc join inasistencias as ina on insc.documento = ina.documento and insc.deporte = ina.deporte order by nombre, insc.deporte;
select nombre, insc.deporte, ina.fecha from inscriptos as insc join inasistencias as ina on insc.documento = ina.documento and insc.deporte = ina.deporte where insc.documento = '22222222';
select nombre, insc.deporte, ina.fecha from inscriptos as insc join inasistencias as ina on insc.documento = ina.documento and insc.deporte = ina.deporte where insc.matricula = 's';

--42 - Combinación externa izquierda (left join)
--Primer problema:
if object_id('clientes') is not null
	drop table clientes;
...
--43 - Combinación externa derecha (right join)
--Primer problema:
if object_id('clientes') is not null
	drop table clientes;
...
select c.nombre,domicilio,ciudad, p.nombre from provincias as p right join clientes as c on codigoprovincia = p.codigo;
select c.nombre,domicilio,ciudad, p.nombre from clientes as c left join provincias as p on codigoprovincia = p.codigo;
select c.nombre,domicilio,ciudad, p.nombre from provincias as p right join clientes as c on codigoprovincia = p.codigo where p.codigo is not null;
select c.nombre,domicilio,ciudad, p.nombre from provincias as p right join clientes as c on codigoprovincia = p.codigo where p.codigo is null order by ciudad;

--44 - Combinación externa completa (full join)
--Primer problema:
...
select documento,d.nombre,matricula from inscriptos as i join deportes as d on codigodeporte = codigo;
select documento,d.nombre,matricula from inscriptos as i left join deportes as d on codigodeporte = codigo;
select documento,d.nombre,matricula from deportes as d right join inscriptos as i on codigodeporte = codigo;
select nombre from deportes as d left join inscriptos as i on codigodeporte = codigo where codigodeporte is null;
select documento from inscriptos as i left join deportes as d on codigodeporte = codigo where codigo is null;
select documento,nombre,profesor,matricula from inscriptos as i full join deportes as d on codigodeporte=codigo;

--45 - Combinaciones cruzadas (cross join)
--Primer problema:
...
select m.nombre, m.edad, v.nombre, v.edad from mujeres as m cross join varones as v where m.edad > 40 and v.edad > 40;
select m.nombre, m.edad, v.nombre, v.edad from mujeres as m cross join varones as v where m.edad-v.edad between -10 and 10;
...
select nombre, t.domicilio, descripcion from guardias cross join tareas as t;
select nombre, t.domicilio, descripcion from guardias as g cross join tareas as t where (g.sexo = 'f' and t.descripcion = 'vigilancia interior') or (g.sexo = 'm' and t.descripcion = 'vigilancia exterior');

--46 - Combinación de más de dos tablas
--Primer problema:
...
select s.nombre, d.nombre, anio from deportes as d right join inscriptos as i on codigodeporte = d.codigo left join socios as s on i.documento = s.documento;
...
union
select nombre, domicilio, 'empleado' from empleados order by tipo;

--48 - Agregar y eliminar campos ( alter table - add - drop)
--Primer problema:
if object_id('empleados') is not null
	drop table empleados;
...
alter table empleados add suelo decimal(5,2);
...
--49 - Alterar campos (alter table - alter)
--Primer problema:
if object_id('empleados') is not null
	drop table empleados;
...
alter table empleados alter column nombre varchar(20);

exec sp_columns empleados;

alter table empleados alter column sueldo decimal(6,2) not null;
alter table empleados alter column documento char(8) not null;
...
--51 - Subconsultas como expresión
--Primer problema:
if object_id('alumnos') is not null
	drop table alumnos;
...
select * from alumnos where nota = (select nombre, max(nota) from alumnos);
...
--54 - Subconsultas correlacionadas
--Primer problema:
if object_id('inscriptos') is not null
	drop table inscriptos;
...
select nombre from socios as s where exists (select * from inscriptos as i where s.numero = i.numerosocio and i.deporte = 'natacion');
select nombre from socios as s where not exists (select * from inscriptos as i where s.numero = i.numerosocio and i.deporte = 'natacion');
select s.* from socios as s where exists (select * from inscriptos as i where s.numero = i.numerosocio and i.cuotas = 10);

--56 - Subconsulta en lugar de una tabla
--Primer problema:
if object_id('inscriptos') is not null
	drop table inscriptos;
...
(
	...
	matricula char(1), --s = paga, n = impaga
	primary key(documento, codigodeporte,anio),
	constraint FK_inscriptos_socios foreign key(documento) references socios(documento) on update cascade on delete cascade
);
...
select i.documento, i.codigodeporte, d.nombre as deporte, anio, matricula, d.profesor from deportes as d join inscriptos as i on d.codigo = i.codigodeporte;
select s.nombre, td.deporte, td.profesor, td.anio, td.matricula from socios as s join (select i.documento, i.codigodeporte, d.nombre as deporte, anio, matricula, d.profesor from deportes as d join inscriptos as i on d.codigo = i.codigodeporte) as td on td.documento = s.documento;
			
--57 - Subconsulta (update - delete)
--Primer problema:
if object_id('inscriptos') is not null
	drop table inscriptos;
...
update inscriptos set matricula='s' where numerosocio = (select numero from socios where documento='25555555');

delete from inscriptos where numerosocio in(select numero from socios as s join inscriptos on numerosocio = numero where matricula = 'n');									   

--58 - Subconsulta (insert)
--Primer problema:
...
insert into clientespref select nombre, domicilio from clientes where codigo in (select codigocliente from clientes as c join facturas as f on codigocliente = codigo group by codigocliente having sum(total) > 500);

select * from clientespref;

--59 - Crear tabla a partir de otra (select - into)
--Primer problema:
if object_id('empleados')is not null
	drop table empleados;
...
select socio from vista_club where socio is null and deporte is not null;
...
--61 - Lenguaje de control de flujo (case)
--Primer problema:
if object_id('empleados') is not null
	drop table empleados;
...
select nombre,datepart(year,fechaingreso) as añoingreso, datepart(year,getdate())-datepart(year,fechaingreso) as AñosdeServicio, placa = case (datepart(year,getdate())-datepart(year,fechaingreso)) % 10 when 0 then 'Si' else 'No' end from empleados where datepart(month,fechaingreso) = 4;
...
--62 - Lenguaje de control de flujo (if)
--Primer problema:
if object_id('empleados') is not null
	drop table empleados;
...
--63 - Procedimientos almacenados (crear - ejecutar)
--Primer problema:
if object_id('empleados') is not null
	drop table empleados;
...
--64 - Funciones escalares (crear y llamar)
--Primer problema:
...
--65 - Funciones de tabla de varias instrucciones
--Primer problema:
if object_id('empleados') is not null
	drop table empleados;
...
returns @listado table(documento char(8),nombre varchar(60),domicilio varchar(60),nacimiento varchar(12))
as 
begin
	...
	if @opcion='total'
		insert @listado 
		select documento, (apellido+' '+nombre), (domicilio+' ('+ciudad+')'), cast(fechanacimiento as varchar(12)) from empleados
	else
		insert @listado
